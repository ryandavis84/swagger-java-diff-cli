...
public class AddPathTest {
    ...
    public void setup() throws Exception {
        File file = new File(getClass().getClassLoader().getResource("swaggerAddPath.json").toURI());
    ...
    public void testSetBreakingChange() throws Exception {
        ...
        boolean defaultValue = addPath.isBreakingChange();
        addPath.setBreakingChange(true);
        boolean newValue = addPath.isBreakingChange();
        assertNotEquals(defaultValue, newValue);
    }

    @Test
    public void testIsBreakingChange() throws Exception {
        assertEquals(new AddPath().isBreakingChange(), false);
    }

    @Test
    public void testGetRuleName() throws Exception {
        String ruleName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, AddPath.class.getSimpleName());
        assertEquals(new AddPath().getRuleName(), ruleName);
    }
    ...
    public void testValidateBreakingOn() throws Exception {
        AddPath addPath = new AddPath();
        addPath.setBreakingChange(true);
        List<BrokenRule> brokenRulesList = addPath.validate(swagger, swaggerDeployed);
    ...
    public void testValidateBreakingOff() throws Exception {
        AddPath addPath = new AddPath();
        addPath.setBreakingChange(false);
        List<BrokenRule> brokenRulesList = addPath.validate(swagger, swaggerDeployed);
