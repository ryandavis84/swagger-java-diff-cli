...
public class DeleteDefinitionTest {
    ...
    public void setup() throws Exception {
        File file = new File(getClass().getClassLoader().getResource("swaggerDeleteDefinition.json").toURI());
    ...
    public void testSetBreakingChange() throws Exception {
        DeleteDefinition deleteDefinition = new DeleteDefinition();
        boolean defaultValue = deleteDefinition.isBreakingChange();
        deleteDefinition.setBreakingChange(false);
        boolean newValue = deleteDefinition.isBreakingChange();
        assertNotEquals(defaultValue, newValue);
    }

    @Test
    public void testIsBreakingChange() throws Exception {
        assertEquals(new DeleteDefinition().isBreakingChange(), true);
    }

    @Test
    public void testGetRuleName() throws Exception {
        String ruleName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, DeleteDefinition.class.getSimpleName());
        assertEquals(new DeleteDefinition().getRuleName(), ruleName);
    }

    @Test
    public void testValidateBreakingOn() throws Exception {
        DeleteDefinition deleteDefinition = new DeleteDefinition();
        deleteDefinition.setBreakingChange(true);
        List<BrokenRule> brokenRulesList = deleteDefinition.validate(swagger, swaggerDeployed);
